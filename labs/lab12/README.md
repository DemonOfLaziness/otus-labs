...
Настройку OSPF надо начать с backbone. Решено было R14-15 сделать ABR, но не для всех зон (конкретно для AREA 101 и 102), поэтому в AREA 0 попали уже имеющиеся активные интерфейсы e0/0 и e0/1 обоих маршрутизаторов. Кроме того, решено было добавить дополнительный линк напрямую между R14 и R15 (во-первых, для обмена информацией напрямую, а не через второй маршрутизатор, и во-вторых, на будущее, так как эти роутеры - пограничные с провайдером, и для упрощения распределения трафика и настройки второго протокола дин. маршрутизации, маршрутизаторам нужна прямая связь). Этот линк тоже попадает в AREA 0.  

OSPF был настроен в следующем порядке - сначала включен сам протокол, затем настроен router-id (совпадает с адресом для управления), затем анонсирована подсеть управления (192.168.100.x/26), а потом на каждом линке, входящем в AREA 0, настроен анонс его подсети.  
...
Помимо маршрутизаторов в AREA 10 были добавлены SW4-5 (для нормального доступа к подсети пользователей 192.168.2.0/23).  
...
R15(config)#
R15(config)#router ospf 1
R20(config-router)#area 102 filter-list prefix area_102_in in
```  
