...
Для настройки приоритетного провайдера необходимо поменять local-preferences маршрутов от Ламаса на R15 так, чтобы они были больше, чем local-preference, выставляемый по умолчанию, дабы при обмене с iBGP-соседями эти маршруты были приоритетней таких же, но полученных другим путём. 
